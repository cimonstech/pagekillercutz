...
export async function POST(request: Request) {
  try {
    ...
    const sms = `Hi ${o.customer_name}, your payment for ${o.order_number} is confirmed. We're processing your order. ${trackUrl}`;

    await Promise.allSettled([
      sendSMS(o.customer_phone, sms),
      sendEmail({
        to: o.customer_email,
        subject: `Payment confirmed — ${o.order_number}`,
        html: ET.orderPaymentConfirmedClient({
    ...
    logger.errorRaw("route", "[api/notify/order-payment-confirmed]", err);
